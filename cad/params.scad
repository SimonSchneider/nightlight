...
chamber_depth = 30;    // pixel-to-face standoff; governs how evenly light spreads.
                       // Coupon testing showed 20-30 mm is needed behind a 20 mm
                       // window. The largest continuous lit area here is the sun's
                       // 24 mm core disc — the rays are thin and the moon is a
                       // ~13 mm crescent — so 30 mm gives a standoff/aperture ratio
                       // of 1.25 on the disc, inside the measured range.
                       // Chosen over 40 mm to keep light: illuminance falls with
                       // the square of standoff, so 30 mm puts ~78% more on the
                       // face than 40 mm would. The sun has to compete with a lit
                       // room, so that margin matters more than extra evenness.
...
body_d = wall + chamber_depth;                         // 32.4
